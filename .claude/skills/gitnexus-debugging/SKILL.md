...
A stale graph is actively dangerous when debugging: `gitnexus_query`/`gitnexus_context` answer confidently from code that no longer exists, so you trace a call chain that was already refactored away and chase a phantom root cause.
...
```
- [ ] npx gitnexus status FIRST — "Indexed commit" == HEAD, else analyze (a stale graph traces code that no longer exists)
