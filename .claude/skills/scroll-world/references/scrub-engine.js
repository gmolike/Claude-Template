/* ============================================================================
   ...
   TEARDOWN
     mountScrollWorld() returns a cleanup function. Call it on unmount to remove every
     window listener, cancel the rAF loops, revoke all Blob URLs and remove the DOM this
     mount built — required for React 19 StrictMode (mount → unmount → mount) and any SPA
     route change. E.g. React: useEffect(() => mountScrollWorld(ref.current, cfg), []).
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  if (!N) return function teardown() {};   // teardown: always hand back a callable cleanup

  injectCSS();
  container.classList.add('sw-root');

  // teardown: registries the returned cleanup drains on unmount (fixes React 19 StrictMode
  // double-mount leaks — the listeners, Blob URLs and rAF loop from mount #1 must all go).
  const blobUrls = [];                       // every URL.createObjectURL(), revoked on teardown
  let rafId = 0, readRafId = 0, torn = false;
  ...
  const nav = el('nav', 'sw-nav'); nav.setAttribute('aria-label', 'Sections');   // a11y: distinct name for this landmark
  ...
  route.setAttribute('role', 'navigation'); route.setAttribute('aria-label', 'Scenes');   // a11y: landmark for the dot rail
  ...
  SECTIONS.forEach((s, i) => {
    ...
    dot.setAttribute('aria-label', s.label || ('Scene ' + (i + 1)));   // a11y: accessible name for the icon-only control
    dot.innerHTML = `<span class="sw-route__label">${esc(s.label || '')}</span><i></i>`;
    dot.addEventListener('click', () => jumpTo(i)); route.appendChild(dot); dots.push(dot);

    if (config.nav !== false) {
      const b = el('button', 'sw-nav__item'); b.type = 'button'; b.textContent = s.label || '';   // a11y: explicit button type
  ...
  function loadClip(s) {
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        const blobUrl = URL.createObjectURL(blob);   // teardown: keep the handle so it can be revoked
  ...
  function read() {
    if (torn) return;   // teardown: ignore any scheduled read that lands after unmount
    ...
    if (near !== activeIndex) {
      activeIndex = near;
      dots.forEach((d, k) => { d.classList.toggle('is-active', k === near); setCurrent(d, k === near); });   // a11y: reflect active scene
      nav.querySelectorAll('.sw-nav__item').forEach((n, k) => { n.classList.toggle('is-active', k === near); setCurrent(n, k === near); });   // a11y
  ...
  function raf() {
    if (torn) return;   // teardown: stop the render loop after unmount
    ...
    rafId = requestAnimationFrame(raf);   // teardown: track handle so it can be cancelled
  }
  ...
  // teardown: named handler (removable) + tracked read-rAF handle (cancellable).
  ...
  // a11y: keyboard scene navigation. Arrow / Page keys step between scenes, Home/End jump to
  // the ends. Skips form fields and modifier combos, and reuses jumpTo() — which already
  // honours prefers-reduced-motion (instant vs. smooth scroll), so no extra motion guard is needed.
  function onKeyDown(e) {
    ...
    let to;
    switch (e.key) {
      case 'ArrowDown': case 'ArrowRight': case 'PageDown': to = Math.min(N - 1, from + 1); break;
      case 'ArrowUp':   case 'ArrowLeft':  case 'PageUp':   to = Math.max(0, from - 1); break;
  ...
  // teardown: the cleanup the caller invokes on unmount (React useEffect return, Vue
  // onBeforeUnmount, …). Removes every window listener, cancels both rAF loops, revokes
  // all Blob URLs, releases the video decoders and removes the DOM this mount built.
  // Per-element listeners (dot/nav clicks) go with their removed subtrees. The shared
  // #sw-css <style> is intentionally left in place — other live mounts reuse it.
  function teardown() {
    if (torn) return;
    torn = true;                                       // read()/raf() bail on their next tick
    ...
    SEGMENTS.forEach(s => {
      if (s.video) { try { s.video.pause(); } catch (e) {} s.video.removeAttribute('src'); try { s.video.load(); } catch (e) {} s.video = null; }
    });
    blobUrls.forEach(u => { try { URL.revokeObjectURL(u); } catch (e) {} });
    blobUrls.length = 0;
    [sky, scrollbar, topbar, stage, copylayer, route, hint, track].forEach(n => { if (n && n.parentNode) n.parentNode.removeChild(n); });
  ...
  function setCurrent(node, on) { if (on) node.setAttribute('aria-current', 'true'); else node.removeAttribute('aria-current'); }   // a11y: expose active scene to AT
...
function injectCSS() {
  ...
  /* a11y: visible keyboard focus on every interactive control (WCAG 2.4.7 AA). */
  .sw-nav__item:focus-visible,.sw-route__dot:focus-visible,.sw-btn:focus-visible,.sw-topcta:focus-visible,.sw-brand:focus-visible{outline:3px solid var(--sw-accent);outline-offset:3px;border-radius:10px;}
  .sw-route__dot:focus-visible i{transform:scale(1.4);background:var(--sw-accent);}
  ...
  @media (prefers-reduced-motion:reduce){ .sw-hint i::after{animation:none;} .sw-pt{display:none;}
    .sw-nav__item,.sw-route__dot i,.sw-route__label,.sw-btn{transition:none;} }  /* a11y: kill motion on the interactive controls too */
  `;
