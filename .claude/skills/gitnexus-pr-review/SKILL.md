...
| --- | --- | --- |
| `gitnexus_detect_changes` | Returns the literal **`"No changes detected."`** even with hundreds of changed files — it never checks staleness. | **YES.** In a PR review this reads as "nothing to worry about" and approves the PR on no evidence. It means "I have no idea," not "this PR is safe." |
| `gitnexus_impact` | Returns `{"error": "Target '<x>' not found", "impactedCount": 0, "risk": "UNKNOWN"}` for a symbol it never indexed. | No. It reports an explicit error and `UNKNOWN` — not a false green. Treat `UNKNOWN` as "re-index," never as "no dependents." |

A PR that adds new symbols is the normal case here: on a stale index `impact` will report those new symbols as `not found`/`UNKNOWN`. That is the index talking, not a verdict on the PR.

> **Trap for automation:** `npx gitnexus status` **exits 0 in every case** — for a stale index and for "Repository not indexed." alike. A pre-commit or CI gate written as `gitnexus status || block` never fires and silently passes everything. Parse stdout; do not branch on the exit code.

## Checklist

```
- [ ] npx gitnexus status FIRST — "Indexed commit" == HEAD, else analyze (a stale graph invalidates the whole review)
- [ ] Fetch PR diff (gh pr diff or git diff base...head)
- [ ] gitnexus_detect_changes to map changes to affected execution flows
- [ ] "No changes detected." on a non-empty diff = stale index, NOT an all-clear — stop and re-index
- [ ] gitnexus_impact on each non-trivial changed symbol
- [ ] Review d=1 items (WILL BREAK) — are callers updated?
- [ ] risk: "UNKNOWN" / "not found" = index gap, never report it as LOW
...
| Index not verified fresh at Step 0 | UNKNOWN — report as unreviewed, not as LOW |
...
```
0. npx gitnexus status
   → Indexed commit: a1b2c3d  |  git rev-parse HEAD: a1b2c3d  → fresh, proceed
   (If they differed: npx gitnexus analyze, and only then review.)
...
**Index: fresh at <commit> / STALE — review not evidence-based**
