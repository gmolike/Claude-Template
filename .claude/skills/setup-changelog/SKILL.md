...
> [`assets/version-bump.mjs`](./assets/version-bump.mjs) erkennt `.changeset/config.json` und macht
...
- **Ein vorhandener Per-Commit-Bump-Hook MUSS weg** — entfernen oder auf no-op stellen, bevor
  ...
  der Fall aus Fail-loud-Notiz 5. [`assets/version-bump.mjs`](./assets/version-bump.mjs) **erzwingt
...
- **Deshalb wirkt auch der Changesets-Guard (ADR-0024) nicht über einen agent-core-Hook**, sondern über
  das **ausgelieferte Referenz-Script** und dieses **Rezept**: agent-core kann einen fremden
  ...
  Scripts fährt, muss den Guard beim nächsten Abgleich **selbst nachziehen**.
