...
import { SessionEntry, SessionState, UsageStats, HUE_STEPS } from './session/types';
import {
  defaultSession,
  extractLastField,
  parseLines,
  getRecentFilesFromLog,
  getRecentFilePathsFromLog,
} from './session/jsonlParser';
import { computeUsageFromLogs as computeUsageFromLogsImpl } from './session/usageCompute';
import {
  CLAUDE_PROJECTS_DIR,
...
import { clearEntryTimers, resetTimers } from './session/activityTimers';
import { getInstalledClis as getInstalledClisImpl, CliInfo } from './session/clis';
import { getRecentTokenEvents, TokenEvent } from './session/tokenActivity';

// Re-export types so existing imports (panel.ts, extension.ts) keep working.
export type { ActivityState, SessionEntry, SessionState, UsageStats } from './session/types';
...
export class SessionManager {
  ...
  constructor(private readonly onUpdate: (sessions: Map<string, SessionState>) => void) {
    this.init();
  }
  ...
  private watchSettings(): void {
    if (this.watchers.has(CLAUDE_SETTINGS_PATH)) {
      return;
    }
    try {
  ...
  private seedProjectDir(slug: string, slugDir: string): void {
    ...
    for (const file of jsonlFiles) {
      ...
      try {
        ...
        if (this.globalEffort) {
          state.effort = this.globalEffort;
        }
  ...
  private scanForTitle(filePath: string, entry: SessionEntry): void {
    try {
      ...
      const title =
        extractLastField(content, '"custom-title"', 'customTitle') ??
  ...
  private watchProjectsRoot(): void {
    if (this.watchers.has(CLAUDE_PROJECTS_DIR)) {
      return;
    }
    try {
  ...
  private scheduleUpdate(): void {
    ...
    this.debounceId = setTimeout(() => {
      ...
      try {
        ...
        /* caller errors must not crash us */
      }
    }, DEBOUNCE_MS);
  }
