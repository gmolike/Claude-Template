...
export function extractLastField(
  content: string,
  marker: string,
  field: string,
): string | undefined {
...
export function defaultSession(sessionId: string, slug: string, hueShift: number): SessionState {
  return {
    sessionId,
    slug,
...
export function humanizeToolUse(name: string, input: Record<string, unknown>): string {
  switch (name) {
    ...
    case 'Agent':
      return String(input['description'] || 'Delegating task');
    default:
      return name;
  }
}
  ...
  if (type === 'assistant') {
    ...
    if (stopReason === 'tool_use') {
      ...
      if (textBlocks.length > 0) {
        ...
        const isQuestion =
          lastText.endsWith('?') ||
          /\b(which|would you( like)?|do you want|please (choose|select|let me know)|what would|how would you|shall i|should i)\b/i.test(
            lastText,
          ) ||
...
/**
 * Scan a JSONL file on disk for the most recent Read/Edit/Write file_paths,
 * returning the FULL file paths (not basenames) for FSD layer categorization.
 * Deduplication is performed on the full path.
 *
 * @param filePath - Absolute path to the JSONL session log file
 * @returns Array of up to 50 full file paths, ordered by occurrence (oldest first)
 */
export function getRecentFilePathsFromLog(filePath: string): string[] {
  try {
    const content = fs.readFileSync(filePath, 'utf-8');
    const files: string[] = [];
    const seen = new Set<string>();
