...
function renderSessions(sessions) {
  ...
  const sorted = (sessions || []).slice().sort((a, b) => (b.lastSeen || 0) - (a.lastSeen || 0));
  ...
  if (sorted.length > 0) {
    ...
    if (txt) {
      ...
      if (spaceIdx > 0 && isWorking) {
        ...
        txt.innerHTML =
          verb + ' <span class="action-target">' + target.replace(/</g, '&lt;') + '</span>';
      } else {
        txt.textContent = raw;
      }
    }
...
window.addEventListener('message', (e) => {
  ...
  if (msg.type === 'projectInfo') {
    ...
    if (ws)
      ws.textContent = repos.length > 1 ? repos.length + ' repos' : first?.workspace || '\u2014';
    if (pp) {
      pp.textContent = first?.workspacePath || '';
      pp.title = first?.workspacePath || '';
      pp.dataset.path = first?.workspacePath || '';
    }
    ...
    if (gitBody) {
      if (repos.length === 0) {
        gitBody.innerHTML =
          '<div class="cap-item" style="color:#a0a0a0;">No workspace folders</div>';
      } else {
        gitBody.innerHTML = repos
          .map((r, i) => {
            const repoLink = r.gitRemote
              ? '<a class="link" onclick="vscodeApi.postMessage({type:\'openUrl\',url:\'https://github.com/' +
                r.gitRemote +
                '\'})">' +
                r.gitRemote +
                '</a>'
              : '\u2014';
            const changesLabel =
              r.uncommittedCount > 0 ? r.uncommittedCount + ' uncommitted' : 'Clean';
            const changesColor = r.uncommittedCount > 0 ? 'color:#f59e0b;' : '';
            const header =
              repos.length > 1
                ? '<div class="cli-group-label" style="cursor:pointer;" onclick="vscodeApi.postMessage({type:\'openFolder\',path:\'' +
                  r.workspacePath.replace(/\\/g, '\\\\') +
                  '\'})">' +
                  r.workspace +
                  '</div>'
                : '';
            return (
              header +
              '<div class="stat-row"><span class="stat-label">Repo</span><span class="stat-value">' +
              repoLink +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Visibility</span><span class="stat-value">' +
              (r.isPrivate === true ? 'Private' : r.isPrivate === false ? 'Public' : '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Branch</span><span class="stat-value">' +
              (r.gitBranch || '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Changes</span><span class="stat-value" style="' +
              changesColor +
              '">' +
              changesLabel +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Ahead/Behind</span><span class="stat-value">\u2191' +
              (r.ahead || 0) +
              ' \u2193' +
              (r.behind || 0) +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Last Commit</span><span class="stat-value" title="' +
              (r.gitLastCommit || '') +
              '">' +
              (r.gitLastCommit || '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Committed</span><span class="stat-value">' +
              fmtDate(r.lastCommitDate) +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Commits</span><span class="stat-value">' +
              (r.totalCommits > 0 ? r.totalCommits : '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Contributors</span><span class="stat-value">' +
              (r.contributors > 0 ? r.contributors : '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Branches</span><span class="stat-value">' +
              (r.branchCount > 0 ? r.branchCount : '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Stars</span><span class="stat-value">' +
              (r.stars != null ? r.stars : '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">Issues</span><span class="stat-value">' +
              (r.openIssues != null ? r.openIssues : '\u2014') +
              '</span></div>' +
              '<div class="stat-row"><span class="stat-label">PRs</span><span class="stat-value">' +
              (r.openPRs != null ? r.openPRs : '\u2014') +
              '</span></div>' +
              (i < repos.length - 1
                ? '<div style="border-bottom:1px solid var(--vscode-panel-border,#333);margin:8px 0;"></div>'
                : '')
            );
          })
          .join('');
  ...
  if (msg.type === 'envData') {
    ...
    if (rfList) {
      if (d.recentFiles && d.recentFiles.length > 0) {
        rfList.innerHTML = d.recentFiles
          .map(
            (f) =>
              '<div class="cap-item"><span class="cap-dot" data-status="detected"></span><a class="link" onclick="vscodeApi.postMessage({type:\'openFile\',file:\'' +
              f +
              '\'})">' +
              f +
              '</a></div>',
    ...
    if (mcpList) {
      if (d.mcpServers && d.mcpServers.length > 0) {
        mcpList.innerHTML = d.mcpServers
          .map(
            (s) =>
              '<div class="cap-item"><span class="cap-dot" data-status="connected"></span><span>' +
              s +
              '</span></div>',
          )
          .join('');
    ...
    if (shList) {
      if (d.recentSessions && d.recentSessions.length > 0) {
        shList.innerHTML = d.recentSessions
          .map((s) => {
            const ago = fmtAgo(s.lastSeen);
            const dot = ACTIVE_STATES.has(s.activity)
              ? 'connected'
              : s.activity === 'sleeping'
                ? 'no'
                : 'detected';
            ...
            return (
              '<div class="cap-item" style="justify-content:space-between;"><span style="display:flex;align-items:center;gap:6px;min-width:0;"><span class="cap-dot" data-status="' +
              dot +
              '"></span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
              title +
              '</span></span><span style="color:#a0a0a0;font-size:10px;flex-shrink:0;">' +
              ago +
              '</span></div>'
            );
          })
  ...
  if (msg.type === 'scrumTasks') {
    _allScrumTasks = msg.tasks || [];
    initScrumFilters();
    renderScrumBoard();
  }
  ...
  if (msg.type === 'fsdActivity') {
    renderFsdLayers(msg.layers || []);
  }
});
...
function refreshUsage() {
  ...
  [sessBar, weekBar].forEach((el) => {
    if (el) {
      el.style.transition = 'none';
      el.style.width = '0%';
    }
  });
  [sessMarker, weekMarker].forEach((el) => {
    if (el) {
      el.style.transition = 'none';
      el.style.left = '0%';
    }
...
function updateUsageMeters(usage) {
  ...
  if (usage.live) {
    if (sessVal)
      sessVal.innerHTML =
        (sessReset
          ? '<span style="color:var(--text-muted);font-weight:400;font-size:9px;">' +
            sessReset +
            '</span> '
          : '') +
        sessPct +
  ...
  const sessTimePct = usage.live
    ? Math.max(
        0,
        Math.min(
          100,
          ((usage.sessionWindowMs - usage.sessionResetMs) / usage.sessionWindowMs) * 100,
        ),
      )
    : sessPct;
  const weekTimePct = usage.live
    ? Math.max(
        0,
        Math.min(100, ((usage.weeklyWindowMs - usage.weeklyResetMs) / usage.weeklyWindowMs) * 100),
      )
    : weekPct;
  ...
  if (sessBar) {
    sessBar.style.width = Math.min(100, sessPct) + '%';
    sessBar.dataset.level = sessLevel;
  }
  if (weekBar) {
    weekBar.style.width = Math.min(100, weekPct) + '%';
    weekBar.dataset.level = weekLevel;
  }
...
function xAxisTicks(windowHours) {
  if (windowHours < 0.1)
    return [
      { hoursAgo: 5 / 60, label: '5m ago' },
      { hoursAgo: 2.5 / 60, label: '2m' },
      { hoursAgo: 0, label: 'now' },
    ];
  if (windowHours < 0.3)
    return [
      { hoursAgo: 15 / 60, label: '15m ago' },
      { hoursAgo: 10 / 60, label: '10m' },
      { hoursAgo: 5 / 60, label: '5m' },
      { hoursAgo: 0, label: 'now' },
    ];
  if (windowHours < 0.6)
    return [
      { hoursAgo: 30 / 60, label: '30m ago' },
      { hoursAgo: 20 / 60, label: '20m' },
      { hoursAgo: 10 / 60, label: '10m' },
      { hoursAgo: 0, label: 'now' },
    ];
  if (windowHours <= 1)
  ...
  if (windowHours <= 12)
    return [
      { hoursAgo: 12, label: '12h ago' },
      { hoursAgo: 8, label: '8h' },
      { hoursAgo: 4, label: '4h' },
      { hoursAgo: 0, label: 'now' },
    ];
  return [
    { hoursAgo: 24, label: '24h ago' },
    { hoursAgo: 16, label: '16h' },
    { hoursAgo: 8, label: '8h' },
    { hoursAgo: 0, label: 'now' },
  ];
}
...
function renderTokenActivity(events) {
  ...
  const labelColor = (
    getComputedStyle(document.body).getPropertyValue('--vscode-descriptionForeground') || '#6e7681'
  ).trim();
  ...
  barGrad.addColorStop(0.0, '#047857'); // green at baseline
  barGrad.addColorStop(0.5, '#16a34a'); // still green at mid
  barGrad.addColorStop(0.75, '#eab308'); // yellow kicks in around 75%
  barGrad.addColorStop(0.92, '#f59e0b'); // orange nearer the top
  barGrad.addColorStop(1.0, '#ef4444'); // red right at the peak
  ...
  const lineColor = (
    getComputedStyle(document.body).getPropertyValue('--vscode-foreground') ||
  ...
  if (totalEl) {
    totalEl.textContent =
      fmtTokensShort(totalTokens) + ' tokens \u00b7 ' + visible.length + ' msgs';
  }
}
...
new MutationObserver(() => renderTokenActivity(undefined as any)).observe(document.body, {
  attributes: true,
...
function pickAnim(activity, lastAction) {
  ...
  if (activity === 'tooling') {
    ...
    if (act.includes('install') || act.includes('build') || act.includes('npm'))
      return 'crouchRunCarry';
    ...
    if (act.includes('save') || act.includes('download') || act.includes('export'))
      return 'crouchLand';
...
// ─── Agent Orchestration ───────────────────────────────────────────────────
function renderAgents(agents: { name: string; domain: string; active: boolean }[]) {
  const list = document.getElementById('agent-list');
  if (!list) return;

  if (!agents || agents.length === 0) {
    list.innerHTML = '<div class="cap-item" style="color:#a0a0a0;">No tmux sessions</div>';
    return;
  }

  list.innerHTML = agents
    .map((a) => {
      const dot = a.active ? 'connected' : 'no';
      return (
        '<div class="cap-item">' +
        '<span class="cap-dot" data-status="' +
        dot +
        '"></span>' +
        '<span>' +
        a.domain +
        '</span>' +
        '<span style="color:var(--text-muted);font-size:10px;margin-left:auto;">' +
        (a.active ? 'attached' : 'detached') +
        '</span>' +
        '</div>'
      );
    })
...
// ─── FSD Layer Monitor ─────────────────────────────────────────────────────
const FSD_LAYER_COLORS: Record<string, string> = {
  app: '#ef4444',
  routes: '#f59e0b',
  pages: '#eab308',
  widgets: '#22c55e',
...
/**
 * Render FSD layer activity into the `fsd-layers-list` container.
 *
 * @param layers - Array of FSD activity records from the extension host
 */
function renderFsdLayers(layers: { layer: string; files: string[]; count: number }[]) {
  const list = document.getElementById('fsd-layers-list');
  if (!list) return;

  if (!layers || layers.length === 0) {
    list.innerHTML = '<div class="cap-item" style="color:#a0a0a0;">No FSD files touched</div>';
    return;
  }
  ...
  list.innerHTML = layers
    .map((l) => {
      ...
      const color = FSD_LAYER_COLORS[l.layer] || '#6b7280';
      const filesTitle = l.files.join(', ');
      return (
        '<div class="cap-item" style="display:flex;flex-direction:column;gap:2px;" title="' +
        filesTitle +
        '">' +
        ...
        '</span>' +
        '</div>' +
        '<div class="context-bar-track" style="height:4px;">' +
        '<div style="width:' +
        pct +
...
function ensureCategoryChips() {
  ...
  const allBtn =
    '<button class="skills-category-btn" data-category="all" data-active="true">All</button>';
  row.innerHTML =
    allBtn +
    shown
      .map(
        (c) =>
          '<button class="skills-category-btn" data-category="' +
          c +
          '" data-active="false">' +
          c +
          '</button>',
...
function renderSkills() {
  ...
  list.innerHTML = filtered
    .map((s) => {
      ...
      const desc = s.description ? '<div class="skill-desc">' + s.description + '</div>' : '';
      return (
        '<div class="skill-item" data-source="' +
        s.source +
        "\" onclick=\"vscodeApi.postMessage({type:'inputSkill',name:'/" +
        s.name +
        '\'})"><div><span class="skill-name">/' +
        s.name +
        '</span>' +
        badge +
        '</div>' +
...
// ─── Scrum Board ───────────────────────────────────────────────────────────
let _allScrumTasks: any[] = [];
let _scrumDomainFilter = 'all';
...
function renderScrumBoard() {
  const container = document.getElementById('scrum-tasks-container');
  if (!container) return;

  const filtered = _allScrumTasks.filter((t) => {
    if (_scrumDomainFilter !== 'all' && t.domain !== _scrumDomainFilter) return false;
    return true;
  });

  if (filtered.length === 0) {
    container.innerHTML = '<div class="cap-item" style="color:#a0a0a0;">No tasks</div>';
    return;
  }

  // Group by status
  const groups: Record<string, any[]> = { 'in-progress': [], backlog: [], done: [] };
  ...
  for (const [status, tasks] of Object.entries(groups)) {
    ...
    html +=
      '<div class="cli-group-label">' +
      SCRUM_STATUS_LABELS[status] +
      ' (' +
      tasks.length +
      ')</div>';
    for (const t of tasks) {
      const dot = SCRUM_STATUS_COLORS[t.status] || 'no';
      const nextStatus =
        t.status === 'backlog' ? 'in-progress' : t.status === 'in-progress' ? 'done' : '';
      const actionBtn = nextStatus
        ? '<button class="card-refresh-btn" title="Move to ' +
          SCRUM_STATUS_LABELS[nextStatus] +
          "\" onclick=\"event.stopPropagation();vscodeApi.postMessage({type:'moveTask',filePath:'" +
          t.filePath.replace(/\\/g, '\\\\').replace(/'/g, "\\'") +
          "',newStatus:'" +
          nextStatus +
          '\'});">&rarr;</button>'
        : '';
      html +=
        '<div class="cap-item" style="justify-content:space-between;">' +
        ...
        t.title +
        '</span>' +
        '<span style="color:var(--text-muted);font-size:9px;flex-shrink:0;">' +
        t.domain +
        '</span>' +
        '</span>' +
        actionBtn +
...
function initScrumFilters() {
  const filterRow = document.getElementById('scrum-domain-filter');
  if (!filterRow) return;

  const domains = new Set(_allScrumTasks.map((t) => t.domain).filter(Boolean));
  ...
  filterRow.innerHTML =
    ...
    Array.from(domains)
      ...
      .map(
        (d) =>
          '<button class="skills-filter-btn" data-filter="' +
          d +
          '" data-active="false">' +
          d +
          '</button>',
  ...
  filterRow.querySelectorAll('.skills-filter-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      filterRow
        .querySelectorAll('.skills-filter-btn')
        .forEach((b) => ((b as HTMLElement).dataset.active = 'false'));
      (btn as HTMLElement).dataset.active = 'true';
      _scrumDomainFilter = (btn as HTMLElement).dataset.filter || 'all';
      renderScrumBoard();
    });
  });
}
    ...
    .map((group) => {
      const items = groups[group]
        .map((c) => {
          ...
          if (c.installed) {
            return (
              '<div class="cap-item" style="cursor:pointer;" onclick="vscodeApi.postMessage({type:\'inputSkill\',name:\'' +
              c.name +
              '\'})"><span class="cap-dot" data-status="' +
              status +
              '"></span><span>' +
...
function restoreLayoutState() {
  ...
  if (state.sectionOrder) {
    const sections = {};
    root.querySelectorAll('.section[draggable]').forEach((s) => {
      sections[s.id] = s;
    });
...
document.querySelectorAll('.section[draggable]').forEach((section) => {
  ...
  section.addEventListener('dragover', (e) => {
    ...
    if (section !== _draggedSection) {
      document
        .querySelectorAll('.section.drag-over')
