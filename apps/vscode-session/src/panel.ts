...
import { getScrumTasks, moveTask, ScrumTaskStatus } from './session/scrumBoard';
import { getTmuxSessions } from './session/agentOrchestration';
import { categorizeFsdFiles } from './session/fsdLayerMonitor';
...
export class Panel implements vscode.WebviewViewProvider {
  ...
  private handleWebviewMessage(msg: WebviewToExtension): void {
    ...
    if (msg.type === 'refreshTokenActivity') {
      if (this.onRefreshTokenActivityCallback) {
        this.onRefreshTokenActivityCallback();
      }
    }
    ...
    if (msg.type === 'moveTask' && msg.filePath && msg.newStatus) {
      const task = { filePath: msg.filePath } as Parameters<typeof moveTask>[0];
      moveTask(task, msg.newStatus as ScrumTaskStatus);
      this.sendScrumTasks();
    }
    if (msg.type === 'refreshScrum') {
      this.sendScrumTasks();
    }
  }
  ...
  onRefreshTokenActivity(cb: () => void): void {
    this.onRefreshTokenActivityCallback = cb;
  }
  ...
  /**
   * Categorizes the given file paths by their FSD layer and posts the result
   * to the webview as a `fsdActivity` message.
   *
   * @param files - Full file paths from the current session log
   */
  sendFsdActivity(files: string[]): void {
    const layers = categorizeFsdFiles(files);
    this.postMessage({ type: 'fsdActivity', layers });
  }

  /**
   * Reads Scrum tasks from the workspace's `.scrum/` directory and posts
   * them to the webview.  A missing workspace or empty board results in an
   * empty task list being sent (no error thrown).
   */
  sendScrumTasks(): void {
    const folders = vscode.workspace.workspaceFolders ?? [];
    if (folders.length === 0) {
      this.postMessage({ type: 'scrumTasks', tasks: [] });
      return;
    }
    const allTasks = folders.flatMap((f) => getScrumTasks(f.uri.fsPath));
    this.postMessage({ type: 'scrumTasks', tasks: allTasks });
  }

  /**
   * Queries active tmux sessions via `getTmuxSessions()` and posts the result
   * to the webview as an `agentStatus` message.
   */
  sendAgentStatus(): void {
    const agents = getTmuxSessions();
    this.postMessage({ type: 'agentStatus', agents });
  }
  ...
  private collectRepoInfo(folder: vscode.WorkspaceFolder): import('./shared/messages').RepoInfo {
    ...
    const gitLastCommit = git('git log -1 --format=%s');
    const lastCommitDate = git('git log -1 --format=%ai');
    const status = git('git status --porcelain');
    const uncommittedCount = status ? status.split('\n').length : 0;
    const ahead = parseInt(git('git rev-list @{u}..HEAD --count'), 10) || 0;
    const behind = parseInt(git('git rev-list HEAD..@{u} --count'), 10) || 0;
    const totalCommits = parseInt(git('git rev-list --count HEAD'), 10) || 0;
    const contribOut = git('git shortlog -sn --all');
    const contributors = contribOut ? contribOut.split('\n').length : 0;
    const stashOut = git('git stash list');
    const stashCount = stashOut ? stashOut.split('\n').length : 0;
    const branchOut = git('git branch -a');
    const branchCount = branchOut ? branchOut.split('\n').length : 0;
    const tagCount = parseInt(git('git tag -l | wc -l'), 10) || 0;

    const ghJson = git(
      'gh repo view --json isPrivate,stargazerCount,forkCount,pushedAt,createdAt,diskUsage,issues,pullRequests 2>/dev/null',
    );
    if (ghJson) {
      try {
  ...
  sendProjectInfo(): void {
    ...
    const repos = folders.map((f) => this.collectRepoInfo(f));

    this.postMessage({
      type: 'projectInfo',
      data: {
        activeFile: editor ? vscode.workspace.asRelativePath(editor.document.uri) : '',
        repos,
      },
    });
  }
  ...
  private static readWebviewAsset(name: string): string {
    if (!Panel._cachedAssets[name]) {
      Panel._cachedAssets[name] = fs.readFileSync(path.join(__dirname, 'webview', name), 'utf8');
    }
  ...
  private buildHtml(webview: vscode.Webview): string {
    ...
    const bundleUri = webview
      .asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'out', 'webview', 'bundle.js'))
      .toString();
